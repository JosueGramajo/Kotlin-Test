...
class RecyclerViewAdapter(val list: List<Activities>, val itemClick : (Activities) -> Unit)
    : RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder>(){

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) : ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.utlis_recycler_item_row,parent,false)
    ...
    class ViewHolder(itemView: View, val itemClick : (Activities) -> Unit) : RecyclerView.ViewHolder(itemView){
        fun bind(activities : Activities) = with(itemView){
            val tv_nombre = itemView.findViewById(R.id.row_name) as TextView
            val tv_descripcion = itemView.findViewById(R.id.row_description) as TextView
